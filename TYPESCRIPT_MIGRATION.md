...
- [x] Phase 4 — strict mode, remaining tests/scripts as appropriate, final CI gate
...
Completed 2026-09-02: cleared the final comparison/controller diagnostics and enabled full
`strict` mode in `tsconfig.json`. All browser application modules in `src/` and `csconfs/` are
TypeScript except `src/csrankings-rules.generated.js`, which is generated by the upstream-rule
sync and remains JavaScript by design. The JavaScript unit/E2E tests, Vite/Playwright configs,
and operational/manual scripts remain JavaScript because Node executes them directly and they
are outside the browser-application migration boundary. CI continues to require unit tests,
strict typechecking, and a production build. Final verification passes with zero TypeScript
diagnostics, 8 unit test files, 88 bundled modules, and all 38 Playwright browser tests.
