...
two researchers, two subfields, or two venues; a funding view over NSF awards
attributed to the same roster; and applicant-facing context through conference
deadlines and a curated explorer for CS research awards and fellowships.
...
median department moves 18 positions across 20 defensible settings, and 78.9\%
...
\begin{itemize}
  \item \textbf{A rank stability sweep} (\S\ref{sec:stability}) that recomputes
    a department's rank under all 20 combinations of four look-back windows and
    five venue sets, holding region fixed, and reports the resulting envelope
  ...
  \item \textbf{Head-to-head comparison} (\S\ref{sec:comparemethod}) of two
    departments, two researchers, two subfields, or two venues, reporting
    per-area margins and, for departments, a decomposition of the rank gap
    into the areas that produced it.
  ...
  \item \textbf{Applicant-facing context} through a maintained conference
    schedule, a curated CS research awards and fellowships explorer, and
    shareable URLs that preserve the filters and view state behind a result.
...
rankings~\cite{coreportal} draw the line elsewhere. \sys{} supports five sets
(\S\ref{sec:venuesets}), and defaults to the broadest of them.
...
Two additional applicant-facing datasets are maintained locally. Conference
schedule records cover current and upcoming editions, including multiple
submission cycles and rolling deadlines, and are linked to official conference
or sponsoring-organization pages. The grants explorer uses a curated snapshot
of CS research awards, fellowships, and related opportunities. Neither dataset
is used in the publication ranking; they provide decision context around the
research and funding signals.
...
\sys{} supports five venue sets: \csr{}'s default list; \csr{}'s extended list
including additional venues; CORE~A$^*$ only; CORE~A together with A$^*$; and
an All (Union) set combining every venue from the other four. A venue that
upstream scrapes but assigns to no research area is counted by none of them,
matching \csr{}'s own behavior. All (Union) is the default: a first-time
reader is shown the most inclusive view rather than being asked to pick a
methodological stance before they know what any of the choices mean, and can
narrow to \csr{}'s own default set explicitly to reproduce csrankings.org.

\subsection{Rank stability}
\label{sec:stability}

Let $W = \{5, 10, 20, 30\}$ be look-back windows in years and $V$ the five
venue sets. For a fixed region $r$ and end year $y_1$, the sweep evaluates the
pipeline at every $(w, v) \in W \times V$, yielding 20 complete rankings. For a
...
researchers and departments active in both. The same procedure applies to a
pair of individual venues rather than top-level areas, so a reader can compare
two conferences directly (e.g.\ \texttt{PLDI vs ICSE}) instead of only the
subfields they belong to.
...
The application comprises five user-facing workflows --- search and analysis
(including the Discoveries view), a ranking-impact simulator, a conference
schedule, an NSF funding explorer, and a CS research awards and fellowships
explorer --- each with its own entry module, sharing URL-aware controls,
autocomplete, accessibility conventions, and visual components. Submission and
correction forms accompany the conference and grants workflows. The canonical
publication-data load is memoized, so modules loaded together share a single
download and parse.
...
Conference schedules and the grants snapshot are maintained as local,
reviewable data rather than fetched from a runtime service.
...
two columns. A ``Per capita'' filter-bar toggle switches the departmental
column between the \csr{} score and the per-faculty ordering of
\S\ref{sec:percapita}; the toggle is offered only on the pages where a
ranking is the thing being shown --- Search, the discoveries feed, and the
simulator --- and not on the funding page, since a filtered result list is not
a ranking. On Discoveries the same setting re-ranks the university-level
insight calculators; on the simulator it re-ranks the rank-impact estimate
shown for a hypothetical roster change.
...
departments, two researchers, two subfields, or two individual venues; mixed-type
comparisons are refused with an explanation rather than silently coerced. The
view renders a verdict, the scoreboard, and the per-area margin chart. The
rank-gap decomposition of Eq.~\ref{eq:gap} is not surfaced as a separate
interface card --- an earlier version showed it directly, but it restated the
scoreboard's ``areas led'' row without adding information a reader could act
on --- and instead is what this paper's own worked examples in
\S\ref{sec:comparison} are computed with, offline, from the same function.
...
\subsection{Conference schedule and grants explorer}

The conference page searches venues and research areas using the same
autocomplete conventions as the publication views. It exposes conference-year
ranges, shared venue sets, upcoming-only filtering, and multiple submission
cycles, while keeping the source record and official event link visible. A
separate grants page searches curated CS research awards, fellowships, and
related opportunities by audience, sponsor, topic, deadline, and sort order.
Both pages encode their state in the URL. Their submission forms route proposed
records and corrections through review rather than silently changing the public
snapshot.
...
\begin{table}[t]
\centering
\caption{Rank spread (worst minus best position) across 20 settings --- four
look-back windows $\times$ five venue sets --- for 190 ranked U.S.\
...
All ranked departments   & 190 & 18.0 \\
\midrule
\multicolumn{3}{l}{\emph{Distribution over all departments}} \\
First quartile of spread   & \multicolumn{2}{r}{11 positions} \\
Median spread              & \multicolumn{2}{r}{18 positions} \\
Third quartile of spread   & \multicolumn{2}{r}{25 positions} \\
Maximum spread             & \multicolumn{2}{r}{46 positions} \\
Departments moving $>$10   & \multicolumn{2}{r}{78.9\%} \\
Departments moving $>$25   & \multicolumn{2}{r}{24.2\%} \\
\bottomrule
\end{tabular}
\end{table}

The headline number is the median spread of 18 positions, and the fact that
...
of 5 positions; the field as a whole moves a median of 18.
...
\end{axis}
\end{tikzpicture}
\caption{Rank spread across the 20-setting sweep, over 190 ranked U.S.\
...
baseline top 10, only 5 remain in the top 10 under all 20 settings.

Table~\ref{tab:examples} gives individual envelopes.

\begin{table}[t]
\centering
\caption{Rank envelopes for selected departments across the 20 settings.}
\label{tab:examples}
...
under every setting tested. GMU ranges from 23rd to 51st depending on
...
\csr{} default & All (Union)   & 0.982 & 2 & 18 & 9 \\
\csr{} all     & CORE A$^*$    & 0.952 & 5 & 19 & 8 \\
\csr{} all     & CORE A/A$^*$  & 0.973 & 3 & 15 & 10 \\
\csr{} all     & All (Union)   & 0.997 & 0 & 8  & 10 \\
CORE A$^*$     & CORE A/A$^*$  & 0.961 & 4 & 15 & 8 \\
CORE A$^*$     & All (Union)   & 0.951 & 5 & 19 & 8 \\
CORE A/A$^*$   & All (Union)   & 0.974 & 3 & 15 & 10 \\
\bottomrule
\end{tabular}
\end{table}
...
positions, and changes the membership of the top 10. All (Union) is the
outlier in the other direction: because it is a superset of \csr{}'s extended
list, it agrees with \csr{} all almost exactly ($\tau = 0.997$, a median
displacement of 0 positions) --- the venues CORE contributes on top of \csr{}'s
own lists move few departments enough to matter at this window.

Table~\ref{tab:toptens} makes the last point concrete by printing all five
...
\centering
\small
\caption{The U.S.\ top ten under each of the five venue sets, 10-year window.
...
\toprule
& \csr{} default & \csr{} all & CORE A$^*$ & CORE A/A$^*$ & All (Union) \\
\midrule
1  & CMU  & CMU  & CMU  & CMU  & CMU \\
2  & UIUC & UIUC & UIUC & UIUC & UIUC \\
3  & UCSD & UCSD & UCSD & UCSD & UCSD \\
4  & Georgia Tech & Georgia Tech & MIT & Georgia Tech & Georgia Tech \\
5  & MIT  & UW \& MIT & Georgia Tech & MIT & UW \\
6  & UW \& Berkeley & --- & UMich & \emph{UMD} & MIT \\
7  & --- & \emph{UMD} & Berkeley & UMich & \emph{UMD} \\
8  & \emph{UMD} & Berkeley & \emph{Cornell} \& UW & UW & \emph{Northeastern} \& Berkeley \\
9  & UMich & \emph{Northeastern} & --- & Berkeley & --- \\
10 & \emph{Stanford} & UMich & \emph{Stanford} \& \emph{Purdue} & \emph{Northeastern} & UMich \\
\bottomrule
\end{tabular}
\end{table}
...
unsettled. UMD appears in four of the five top tens but never under CORE~A$^*$.
Stanford is 10th under the default set and under CORE~A$^*$ but appears in
none of the other three. Northeastern is 9th under \csr{}'s full venue list,
10th under CORE~A/A$^*$, and tied for 8th under All (Union), but is outside
the top ten under the default set and under CORE~A$^*$. Cornell and Purdue
appear only under CORE~A$^*$. A reader who has internalized ``the top ten'' as a stable
...
\paragraph{Applicant-context data changes.} Conference dates, submission
deadlines, awards, fellowships, and grant opportunities are maintained records,
not stable measurements. They can become outdated or change at the source, so
the schedule and grants workflows expose source links and review caveats rather
than presenting them as guaranteed opportunities.
...
information --- which subfield, which faculty, which funding, which conference
deadlines, and which research opportunities --- that the decision actually
turns on.
